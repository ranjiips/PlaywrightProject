...
To avoid storing plain-text credentials in the repository, we use AES-256-CBC encryption with a secret key. Passwords are encrypted once, stored safely, and decrypted only at runtime.
1. Setup
...
2. Utility File (src/utils/cryptoUtils.ts)
...
3. Helper Script (src/utils/encryptDecryptHelper.ts)
import { encrypt, decrypt } from './cryptoUtils';

const password = "Hjspass#1"; // Replace with your password
...
Run it:
npx ts-node src/utils/encryptDecryptHelper.ts

4. Environment Variables
Create a .env file (never push this to Git — add it to .gitignore):
SECRET_KEY=mySuperSecretKey
ENCRYPTED_PASSWORD=<paste encrypted value here>
USERNAME=

Provide a .env.example file in Git for onboarding:
SECRET_KEY=yourSecretKeyHere
ENCRYPTED_PASSWORD=yourEncryptedPasswordHere
